const Navbar = ({ darkMode, toggleDark, activeTab, setActiveTab }) => {
  return (
    <nav className={`px-8 h-14 flex items-center justify-between border-b ${darkMode ? 'bg-[#13101f] border-[#2a2440]' : 'bg-white border-gray-200'}`}>
      <div className={`flex items-center gap-2 font-medium ${darkMode ? 'text-[#e2dff8]' : 'text-gray-800'}`}>
      ...
      <button onClick={toggleDark}
        className={`text-xs px-3 py-1.5 rounded-lg border transition-colors ${darkMode ? 'border-[#2a2440] text-[#8b85a8] hover:text-white' : 'border-gray-200 text-gray-500 hover:text-gray-800'}`}>
        {darkMode ? '☀ Light' : '☾ Dark'}
      </button>
    </nav>
  )
